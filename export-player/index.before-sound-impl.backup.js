(function() {
  ...
  function normalizeMarkerType(type) {
    if (type === 'info' || type === 'light') { return type; }
    return 'link';
  }
...
function toggleViewerSettings(forceOpen) {
  ...
  async function activateLinkMarker(marker) {
    ...
    switchScene(targetScene, marker.transition || 'fade', getLinkMarkerTargetViewParameters(marker, targetScene));
  }
  ...
  function renderActiveSceneLights() {
    ...
    sceneLights.forEach(function(marker) {
      ...
      var isVisible = screenPosition && Number.isFinite(screenPosition.x) && Number.isFinite(screenPosition.y) && screenPosition.x >= 0 && screenPosition.x <= bounds.width && screenPosition.y >= 0 && screenPosition.y <= bounds.height;
      if (!isVisible) { return; }
  ...
  function syncMarkerHotspot(marker, entry) {
    if (normalizeMarkerType(marker.type) === 'light') {
  ...
  function renderActiveSceneMarkers() {
    ...
    var hotspotMarkers = sceneMarkers.filter(function(marker) { return normalizeMarkerType(marker.type) !== 'light'; });
  ...
  function switchScene(entry, transitionType, viewParameters) {
